...
double info_d(vector<vector<string>> Data,set<int> attribute_list)
{
	...
	{
		count_map[Data[i][class_attribute]]++;
	}
	...
	for(int i=0;i<Data.size();i++)
	{
		...
		{	
			if(map_creating_branches[j].count(Data[i][j])==0)
			{
				...
				vector<vector<string>> v2;v2.clear();
				v2.push_back(v1);
				map_creating_branches[j][Data[i][j]]=v2;
			}
			else 
				map_creating_branches[j][Data[i][j]].push_back(Data[i]);
		}
	}
	double y,gain;
	double mx=-1.0;
...
DTPTR generate_DT(vector<vector<string>> Data,set<int> attribute_list)
{
	...
	for(int i=0;i<rows.size();i++)
	{
		if(map_creating_branches.count(Data[i][best])==0)
		{
			...
			map_creating_branches[Data[i][best]].push_back(rows[i]);
	}
...
int main(){
	...
	output<<"Given Training sample data:"<<endl;
